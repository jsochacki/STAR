function [GAIN PAGLIN] = set_memory_PA_OBO(input, OBO_FROM_P1DB, pa_coefficients, GINIT, Q)

POLYNOMIAL_ORDER = (2*(length(pa_coefficients) / Q)) - 1;
N = length(input);
...
ssvout = Memory_Polynomial_Amplifier(lin_input, pa_coefficients, POLYNOMIAL_ORDER, Q);
...
%FIND P1DB
temp_LIN = input*GAIN;
temp_NLIN = Memory_Polynomial_Amplifier(temp_LIN, pa_coefficients, POLYNOMIAL_ORDER, Q);
...
if OBO_FROM_P1DB
    ...
    GAIN = GAIN - 0.01;
    temp_NLIN = Memory_Polynomial_Amplifier(input*GAIN, pa_coefficients, POLYNOMIAL_ORDER, Q);
    TX_POWER_OUT_dBm_NLIN = 10*log10((temp_NLIN*temp_NLIN')/(N*Z0*0.001));
    end
end
