...
if ~isempty(X1)
   plot(axes1, X1,Y1,'DisplayName',num2str([SNR2, EVM2], 'Receive Data Without Pre-Distortion SNR = %2.2f, EVM = %2.2f'),'Marker','o',...
      'LineStyle','none');
end
if ~isempty(X2)
   plot(axes1, X2,Y2,'DisplayName',num2str([SNR3, EVM3], 'Receive Data With Pre-Distortion SNR = %2.2f, EVM = %2.2f'),'Marker','o',...
      'LineStyle','none');
end
if ~isempty(X3)
